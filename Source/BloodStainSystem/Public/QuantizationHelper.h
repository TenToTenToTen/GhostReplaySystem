...
namespace BloodStainFileUtils_Internal
{
	 ...
	 */
	void SerializeQuantizedTransform(FArchive& Ar, const FTransform& Transform, const FQuantizationOption& QuantOpts, const FRange* Range = nullptr);

	/**
	 * 양자화된 형태로 Ar에 기록된 데이터를 읽어 FTransform으로 복원합니다.
	 */
	FTransform DeserializeQuantizedTransform(FArchive& Ar, const FQuantizationOption& QuantOpts, const FRange* Range = nullptr);
